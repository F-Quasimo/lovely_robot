...
class YoloX:
    def __init__(self, ckpt=os.path.join(exp_dir,'checkpoints/nano_epoch_300_ckpt.pth')):
...
class YoloX:
    ...
    def inference(self, img_in):
        ...
        img_info["raw_img"] = img_in

        ratio = min(self.img_size[0] / img_in.shape[0], self.img_size[1] / img_in.shape[1])
...
if __name__=='__main__':
    ...
    yolox=YoloX()
    output, img_info=yolox.inference(img_in=img)
    vis_ans=yolox.visual(output=output[0],img_info=img_info,cls_conf=0.7)
    cv2.imwrite('dog_out.jpg', vis_ans)
    print(output)
